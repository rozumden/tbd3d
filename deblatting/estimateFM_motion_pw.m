function [f_img m_img roi] = estimateFM_motion_pw(g, b, h, f, m, state, varargin)
...
% NOTE: not updated to _pw version
roi = boundingBox(sum(h,3));
...
% unknowns
if(isempty(f))
	f = zeros(prod(fsize(1:2)), fsize(3)*fsize(4)); % RGB channels of 'f' a column vectors, different objects(apperences) stacked in 2-dim as [RGB1 RGB2 RGB3...]
else
	f = vec3(f); % RGB channels of 'f' a column vectors, different objects(apperences) stacked in 2-dim as [RGB1 RGB2 RGB3...]
end
if(isempty(m))
	m = ones(prod(fsize(1:2)), fsize(4)); % intial mask as column vector, different objects(apperences) stacked in 2-dim as [m1 m2 m3...]
	single_m = false;
else
	single_m = size(m,4) < fsize(4); % assumed 1; sinle mask is const for all object appearances
...
if(~isempty(state))
	...
	% derivatives
	[Dx Dy] = createDerivatives(fsize(1:2)); DTD = Dx.'*Dx + Dy.'*Dy;
end
