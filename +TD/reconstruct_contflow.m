function [res] = reconstruct_contflow(vis)
if ~exist('vis','var')
	vis = false;
end
if vis
...
scl = 16;
myname = '/home.stud/rozumden/denys_rozumnyi';
basename = [myname '/outputs/scale_' int2str(scl) '/'];
nm = 'volleyball';
tp = 'flow_flo';
...
maxn = 10;
for k = 1:maxn
	pth = fullfile(myname, nm, [nm sprintf('_%03d.png', k)]);
	if exist(pth) == 2
		img = im2double(imread(pth));
	else
		break;
	end
	matF = cat(4,matF,img);
	fileName = fullfile(basename, nm, tp, [nm sprintf('_%03d', k) '.flo']);
	if exist(fileName) == 2
		fl = loadFloFile(fileName);
		vx = fl(:,:,1); vy = fl(:,:,2);
		flow = opticalFlow(vx, vy);
		flows = [flows flow];
	else
		fprintf('No flow at %d\n', k);
	end
end
...
M = double(diskMask(size2(matF),0.95*size(matF,1)/2));
...
for kk = iv
	...
	if ~isempty(pnts{kk})
		Vx = double(interp2(flow.Vx, pnts{kk}(:,1), pnts{kk}(:,2)));
		Vy = double(interp2(flow.Vy, pnts{kk}(:,1), pnts{kk}(:,2)));
