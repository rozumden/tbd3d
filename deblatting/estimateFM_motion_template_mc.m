...
do_cost = IF(isfield(params, 'do_cost'), @()params.do_cost, false); % FIXME: not updated to _mc; true = calculate cost function value (and related quantities) in each iteration (takes time ~ 1CG iteration); the result it returned in 'cost' struct; false = don't
...
for i = 1:numel(h)
	...
	g{i} = g{i}(roi(1):roi(2), roi(3):roi(4), :);
	b{i} = b{i}(roi(1):roi(2), roi(3):roi(4), :);
	h{i} = h{i}(roi(1):roi(2), roi(3):roi(4), :);
end
...
for i=1:num_h
	...
	F{i} = zeros(gsize); M{i} = zeros(gsize(1:2)); % prealocation for FT calculation (large psfshifted versions of f/m for conv in FT)
	[idx_large_f{i}] = psfshift_idx(fsize, gsize); % indices for quick conversion between small 'f' (actual 'f') and 'F' (psf-shifted large version of 'f' for conv in FT)
	[idx_large_m{i}] = psfshift_idx(fsize(1:2), gsize(1:2)); % indices for quick conversion between small 'm' (actual 'm') and 'M' (psf-shifted large version of 'm' for conv in FT)
end
...
% precompute FT
H = cell(1,num_h); HT = cell(1,num_h);
for i=1:num_h
	H{i} = fft2(h{i}); HT{i} = conj(H{i});
end

% precompute const RHS for 'f/m' subproblem
rhs_f = zeros(size(f)); rhs_m = zeros(size(m));
for i=1:num_h
...
for iter = 1:maxiter
	...
	% cost
	% FIXME: not updated to _mc version
	if(do_cost)
...
yf = zeros(size(xf)); ym = zeros(size(xm));
for i=1:num_h
	F{i}(idx_large_f{i}) = xf; M{i}(idx_large_m{i}) = xm;

	% forward conv
	HF = H{i}.*fft2(F{i});
	bHM = b{i}.*real(ifft2(H{i}.*fft2(M{i})));

	% transpose conv
	temp = real(ifft2(HT{i}.*(HF - fft2(bHM)))); yf(:) = yf(:) + temp(idx_large_f{i});
	temp = real(ifft2(HT{i}.*fft2(sum(b{i}.*(bHM - real(ifft2(HF))),3)))); ym(:) = ym(:) + temp(idx_large_m{i});
end
